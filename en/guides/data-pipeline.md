...
The [edge preprocessor](/en/references/edge-preprocessor) handles per-modality compression (H.264/H.265 for video, delta encoding for IMU, u-law for audio), applies a hardware timestamp using a monotonic clock synchronized via PTP/NTP, and strips corrupted frames.

The [local buffer](/en/references/local-buffering) (a ring buffer backed by a write-ahead log on flash storage) absorbs bursts during connectivity loss so no data is dropped during handoffs.
